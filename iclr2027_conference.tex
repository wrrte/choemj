...
To assess prediction accuracy, we detached the pre-trained encoder and trained a lightweight multilayer perceptron (MLP) to predict ground-truth Random Access Memory (RAM) states (e.g., diver counts and exact $(x, y)$ coordinates) directly from the latent state $z$. As detailed in Appendix \ref{sec:appendix_detailed_probing}, our probing achieved exceptional accuracy ($>99\%$ for diver counting and $92.4\%$ for diver presence prediction) on pristine clean observations. However, when we injected extreme uniform noise into the background spaces while leaving only the target bounding box intact, the classification accuracy plummeted drastically (e.g., from $99.6\%$ to $68.1\%$ in Seaquest). \todo{noise strength를 여러 단계로 바꾸면서 performance가 어떻게 떨어지는지 graph 그려야함} This drop indicates that the encoder--probe pipeline is sensitive to background corruption. It is consistent with spatial entanglement, but extreme noise also introduces a distribution shift, so this experiment alone does not isolate the cause of the degradation.
