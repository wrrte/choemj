...
### 3.6 자연스러움
앵커와 타겟을 모아 contrastive, 혹은 유사하게 행동하게 강제하지 않고 그저 같은 상상 배치에 넣음.
1. 타겟과 의미적으로 다를 때.
자연스럽게 다른 방향으로 유도가 되어 뭉개지는 대신 두 상황 모두에 적합한 방향으로 자연스럽게 gradient가 유도됨.
2. 타겟과 유사할 때.
td error가 큰 앵커가 유사한 이미지의 타겟과 의미적으로도 유사함을 배울 수 있게 됨.
이 판단 자체가 implicit하게 이루어지게 만들 뿐, 강제하지 않음.
