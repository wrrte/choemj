...
\todo{Across-seed protocol: Extend this analysis to all independent training seeds for each evaluated method and game, reporting the number of training runs separately from the 20 evaluation episodes per checkpoint. Use matched training budgets and evaluation schedules. Define a game-specific success criterion, such as a return threshold, before comparing methods and apply it identically to both. Report the fraction of training runs that ever succeed and the distribution of first-success times to substantiate the claim about unchanged discovery frequency. Distinguish first successes before and after retrieval warmup.}

\todo{Best-training-return versus final-evaluation gap: For each training seed $i$, report $G_i = R^{\mathrm{train,best}}_i - \bar{R}^{\mathrm{eval}}_i(100\mathrm{k})$, where the first term is the maximum completed training episode return over the full budget and the second is the final checkpoint's mean evaluation return. Plot both returns for every training seed with an equality reference line, and report the gap distribution and between-method difference with bootstrap 95\% confidence intervals using training runs as the resampling unit. Document training/evaluation differences in action selection and episode termination. Interpret this gap as descriptive: the selected training maximum and evaluation mean are different statistics, and a smaller gap alone does not establish better exploitation.}

\todo{Retention probability after first success: Record the environment step $t_i^{\mathrm{first}}$ at which each training run first meets the success criterion. At subsequent scheduled checkpoints, estimate the probability that an evaluation episode meets the same criterion. Align curves by environment steps since first success, average these probabilities equally across eligible training runs, and report uncertainty across training seeds, the final-checkpoint success probability, and the number of contributing runs at each offset. Report runs that never discover success separately; do not count them as post-success retention failures.}

\todo{Time-to-consolidation: Prespecify a required evaluation success rate $q$ and a persistence requirement of $K$ consecutive evaluation checkpoints, shared by both methods. Measure the environment steps from first training success until this criterion is confirmed, and report the checkpoint interval, the consolidation fraction, and the time-to-consolidation distribution with uncertainty across training seeds. Treat successful training runs that do not consolidate by 100k as right-censored at their remaining post-success budget rather than dropping them or averaging only completed cases. Add a table and plots summarizing these metrics across seeds alongside the selected-run illustration, and limit conclusions to the observed comparisons.}
...
\todo{Revise the interpretation to acknowledge that the absolute correlation remains weak despite the approximately $2.17\times$ relative increase. Report the absolute effect size, a mean per-sequence Spearman correlation difference of $0.0779$, and avoid treating this result alone as proof of spatial entanglement or its mitigation.}

\todo{Report bootstrap 95\% confidence intervals for each model's mean per-sequence correlation and for the FLASH-minus-Baseline difference; assess whether the difference is statistically distinguishable from zero. If both models use the same background contexts, use paired resampling. Account for temporal dependence in the 17,496 continuous frames through episode-level or block resampling, and document the sampling unit, number of independent episodes, bootstrap repetitions, and interval method. Clarify whether uncertainty covers only evaluation contexts for fixed checkpoints or also variation across training runs.}

\todo{Add the distribution of per-sequence correlations and, for matched contexts, their paired differences in Appendix~\ref{sec:appendix_value_robustness}: include a histogram or violin plot, median and interquartile range, and the fraction of contexts where FLASH improves over Baseline. Define a sequence as the 17 stage variants of one fixed background if this matches the implementation, and report how undefined correlations from constant predictions are handled. Update Table~\ref{tab:value_metrics} with the confidence intervals and summarize the results here.}
