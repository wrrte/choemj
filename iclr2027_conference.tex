...
\REQUIRE Initial collection steps $T_{\text{init}}$ sufficient for replay sampling, total steps $T$
...
\FOR{$t = T_{\text{init}},\ldots,T-1$}
    \STATE Collect a transition into $\mathcal{R}$ and update the hash index (\textbf{Stage 1})
    \STATE Sample a replay batch and update the world model
    \STATE Compute \textit{surprise} $S_u$ with existing EMA, then update EMA (\textbf{Stage 2})
    ...
    \IF{$t \geq T_{\text{warm}}$}
        \STATE Pop up to $M$ anchors; retrieve and re-verify up to $n$ contexts per group (\textbf{Stage 3})
    \ENDIF
    \STATE Fill the remaining batch slots with random contexts (\textbf{Stage 4}, Eq.~\eqref{eq:batch_compose})
    \STATE Generate imagined trajectories from the composed contexts
    \STATE Apply Global Rebuild if due (\textbf{Stage 3} and Warmup Curriculum)
    \STATE Update Actor-Critic with group-weighted losses (\textbf{Stage 4}, Eq.~\eqref{eq:weighted_loss})
\ENDFOR
\end{algorithmic}
\end{algorithm}
