...
Furthermore, we examined the \textit{Global Average Profile} (averaging the value predictions over all 17,496 sequences). The Baseline profile generally increases with igloo construction stage, yielding a high rank correlation ($\rho=0.9632$). FLASH's profile ($\rho=0.6373$) peaks at Stage 11 and is lower at the subsequent stages as the igloo nears completion (Stage 16). As detailed in Appendix~\ref{sec:appendix_value_robustness}, the value increase from Stage 0 to Stage 11 is over twice as large in FLASH ($\sim60.7$) as in the Baseline ($\sim28.5$). Thus, FLASH exhibits a larger value difference across the early-to-mid construction stages and a distinct profile near completion. Its higher average per-sequence rank correlation and lower rank correlation of the global average profile summarize different aspects of these predictions: the former measures ordering within individual background contexts, whereas the latter measures ordering after averaging across contexts.
