...
FLASH retains the base model's encoder architecture while introducing additional computation for hash indexing, anchor selection, retrieval, and memory maintenance. We quantify its practical overhead using total wall-clock training time for STORM with and without FLASH over 100,000 environment steps.
...
Table~\ref{tab:efficiency} reports the total training times on three GPUs. For each GPU, let $t_{\mathrm{base}}$ and $t_{\mathrm{FLASH}}$ denote the measured times for STORM and STORM + FLASH, respectively. The additional time is $\Delta t = t_{\mathrm{FLASH}} - t_{\mathrm{base}}$, and the relative training time overhead is $100 \times \Delta t / t_{\mathrm{base}}\%$. These measurements cover the full 100k-step training budget, including the retrieval warmup described in Section~\ref{subsec:flash_learning}.

\todo{Specify the timed game(s), training seeds, number of runs and aggregation, timing boundaries (including whether evaluation and checkpointing are timed), and shared settings within each GPU comparison.}

\begin{table}[!t]
\centering
\caption{Total wall-clock training time for STORM and STORM + FLASH over 100,000 environment steps on each GPU. Times and absolute increases are reported in minutes; relative overhead is computed against the STORM baseline on the same GPU. TODO entries will be replaced with measured times and the corresponding derived values.}
...
\midrule
% Fill times in minutes; Extra time = FLASH - baseline; Overhead = 100 * Extra time / baseline.
NVIDIA RTX A6000 & 297 & 319 & \todo{--} & \todo{--} \\
NVIDIA RTX PRO 6000 Blackwell Server Edition & 159 & 170 & \todo{--} & \todo{--} \\
NVIDIA GeForce RTX 3090 & 411 & 432 & \todo{--} & \todo{--} \\
\bottomrule
\end{tabular}
\end{table}

For the measured STORM runs, FLASH increases total training time by \todo{A6000 overhead}\% on the NVIDIA RTX A6000, \todo{Blackwell overhead}\% on the NVIDIA RTX PRO 6000 Blackwell Server Edition, and \todo{RTX 3090 overhead}\% on the NVIDIA GeForce RTX 3090 (Table~\ref{tab:efficiency}).
...
In this paper, we critique the recent trend of obsessing over visual representation quality in RL. Our probing experiments show that lightweight encoders can retain substantial task-relevant information despite poor visual reconstruction. This evidence motivates improving how downstream Actor-Critic learners use the available representations, alongside approaches that improve the representations themselves. To validate this alternative approach, we proposed FLASH, a retrieval method designed as a plug-and-play module. By augmenting Actor-Critic training batches with structurally similar past experiences, FLASH implicitly supports downstream learning and improves the overall performance of the evaluated RL baselines. Its measured training time overhead for STORM is reported across three GPUs in Table~\ref{tab:efficiency}.
