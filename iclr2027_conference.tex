...
\begin{figure}[t]
\centering
% Overleaf: upload background_noise_examples.png into experiment/ next to the main TeX file.
\includegraphics[width=\linewidth]{experiment/background_noise_examples.png}
\caption{Background-noise application to actual $64\times64$ observations from Frostbite (top) and Seaquest (bottom). Each row uses one fixed observation; columns show illustrative strengths $\alpha=0,0.25,0.5,0.75,1$. Only the igloo bounding box or the bottom-diver bounding box is preserved. The same random pixel field is used across strengths within each row, and images are enlarged with nearest-neighbor interpolation. No target-stage or diver-count replacement is applied. These illustrative strengths differ from the seven strengths used for the quantitative sweep.}
\label{fig:background_noise_examples}
\end{figure}

\begin{figure}[t]
\centering
% Overleaf: upload experiment/noise_probe_accuracy.pdf.
\includegraphics[width=\linewidth]{experiment/noise_probe_accuracy.pdf}
\caption{Noise-strength sweep with a separately trained probe at each strength and a fixed encoder per game. (a) Overall accuracy, weighted by class frequency. (b) Unweighted mean of class accuracies. Both metrics use the full probing dataset, including probe-training frames. Each point represents one probe training run with seed 42; connecting lines are visual guides, not additional measurements. Validation results and all class-wise accuracies are given in Appendix~\ref{subsec:appendix_noise_sweep}.}
