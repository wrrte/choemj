...
Prior work improves visual representations through reconstruction \citep{hafner2019learning}, contrastive learning (CURL; \citealp{laskin2020curl}), predictive consistency (SPR; \citealp{schwarzer2020data}), decoupled representation learning (ATC; \citealp{stooke2021decoupling}), or prototypical learning (DreamerPro; \citealp{deng2022dreamerpro}). DIAMOND instead uses a diffusion world model to preserve visual detail \citep{alonso2024diffusion}. FLASH investigates better downstream use of compact representations through targeted experience reuse.
...
\paragraph{Hashing and Indexing.}
...
\paragraph{Context Extraction and Batch Composition.}
...
We evaluate this adjustment through comparisons with a variant that preserves the original total weight and a baseline with a reduced random batch (Appendix~\ref{sec:appendix_ablation_weight}).
