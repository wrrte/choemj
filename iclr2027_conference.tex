...
\textbf{Effect of Batch Weighting:} Inverse-group weighting controls the relative contribution of retrieved groups and random contexts to the existing Actor-Critic losses. We evaluate this design empirically through comparisons with a variant that preserves the original total weight and a baseline with a reduced random batch (Appendix~\ref{sec:appendix_ablation_weight}).
...
Table~\ref{tab:efficiency} shows the empirical FPS measurements. FLASH delivers performance gains while keeping the additional computational overhead practically near zero.
...
In this paper, we critique the recent trend of obsessing over visual representation quality in RL. Our probing experiments show that lightweight encoders can retain substantial task-relevant information despite poor visual reconstruction. This evidence motivates improving how downstream Actor-Critic learners use the available representations, alongside approaches that improve the representations themselves. To validate this alternative approach, we proposed FLASH, an efficient and low-overhead retrieval method designed as a plug-and-play module. By augmenting Actor-Critic training batches with structurally similar past experiences, FLASH implicitly supports downstream learning and improves the overall performance of the evaluated RL baselines while maintaining extreme computational efficiency.
...
\section{Ablation on Batch Weight Reduction}
...
\begin{table}[h]
    \centering
    \caption{Global Average Value by Igloo Stage (averaged across 17,496 evaluation frames). The Baseline profile increases monotonically, whereas FLASH peaks at Stage 11 and declines as the igloo nears completion.}
